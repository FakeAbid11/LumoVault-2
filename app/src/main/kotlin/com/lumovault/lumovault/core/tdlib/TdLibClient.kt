 ...
 * isolate, no `@extra` request correlation, no completer map. `Client.execute`
 * correlates responses itself, and updates arrive on a dedicated handler
 * thread.
 *
 ...
 *  - the update stream survives [close] (consumers subscribe once and would be
...
class TdLibClient(
    ...
    @Volatile private var databaseKey: String? = null
    ...
    private val requestHandler = Client.ResultHandler { result ->
        when (result.constructor) {
            ...
            TdApi.UpdateConnectionState.CONSTRUCTOR -> {
                ...
                _updates.tryEmit(Update.ConnectionReady(state.constructor == TdApi.ConnectionStateReady.CONSTRUCTOR))
            }
    ...
    suspend fun initialize(databaseKey: String) {
        synchronized(initLock) {
            ...
            initJob?.let { return }
            initJob = scope.launch { initializeNow(databaseKey) }
        }
        initJob?.join()
    }

    private suspend fun initializeNow(key: String) {
        require(key.isNotEmpty()) { TdLibException(code = "INVALID_DATABASE_KEY", message = "Database key is empty") }
        ...
        val databaseDir = File(base, "tdlib_db").apply { mkdirs() }
        val filesDir = File(base, "tdlib_files").apply { mkdirs() }
     ...
     */
    suspend fun <T : TdApi.Object> send(request: TdApi.Function, expected: Class<T>): T {
        ...
        client.execute(request) { result ->
            if (result.constructor == TdApi.Error.CONSTRUCTOR) {
                deferred.complete(TdLibException.from(result as TdApi.Error))
            } else {
        ...
        val result = try {
            ...
            throw TdLibException(
                ...
                message = "TDLib request timed out: ${request.constructorName}",
            )
        }
        if (result is TdLibException) throw result
        return expected.cast(result)
    }

    /** Fire-and-forget variant for requests whose reply we do not need. */
    fun sendFireAndForget(request: TdApi.Function) {
        client?.execute(request) { /* reply dropped */ }
    }
    ...
    private fun maybeAnswerWaitParameters(state: TdApi.AuthorizationState) {
        if (state.constructor != TdApi.AuthorizationStateWaitTdlibParameters.CONSTRUCTOR) return
    ...
    suspend fun getAuthorizationState(): TdApi.AuthorizationState =
        send(TdApi.GetAuthorizationState(), TdApi.AuthorizationState::class.java)

    suspend fun isAuthenticated(): Boolean = try {
        getAuthorizationState().constructor == TdApi.AuthorizationStateReady.CONSTRUCTOR
    } catch (_: Exception) {
